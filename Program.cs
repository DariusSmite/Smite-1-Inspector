...
namespace SmiteGodLab
{
    ...
    class MainForm : Form
    {
        ...
        Panel BuildCodexPanel()
        {
            ...
            H3("Limitations");
            P("A few things the API cannot do, by design. It cannot reveal a privacy-flagged player's name or id from a completed match or a friends list (only a live match exposes names, and even then not hard-private profiles). It does not expose newer in-client avatars, only an older avatar set, so many active players return no avatar (the app shows a coloured initial instead). A player's match history is capped at about the 50 most recent games with no way to page further back, so for very active players Recent Matches may cover only the last few days and older games (including custom lobbies) drop off. And it does not resolve linked-account names beyond the primary account. These are server-side limits, not app bugs.");
